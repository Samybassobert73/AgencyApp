...
export function getInterventionsByAgencyId(agencyId: string): Intervention[] {
  return getInterventions().filter(
    (intervention) => intervention.agencyId === agencyId
  );
}

export function getInterventionsByContractorId(
  contractorId: string
): Intervention[] {
  return getInterventions().filter(
    (intervention) => intervention.contractorId === contractorId
  );
}

export function createIntervention(
  intervention: Omit<Intervention, "id" | "status" | "createdAt" | "updatedAt">
): Intervention {
